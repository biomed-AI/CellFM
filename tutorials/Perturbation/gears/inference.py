...
def evaluate(loader, model, uncertainty, device, adata):
    """
    ...
    ctrl_x = np.array(adata[adata.obs['condition'] == 'ctrl'].X.todense().mean(0))
    ctrl = torch.tensor(ctrl_x)
    ...
    results['ctrl_de']= ctrl_de.detach().cpu().numpy()
...
def compute_metrics(results, gene_idx=None):
    ...
    for pert in np.unique(results['pert_cat']):
        ...
        for m, fct in metric2fct.items():
            if m == 'pearson':
                val = fct(results['pred'][p_idx].mean(0) - results['ctrl'], results['truth'][p_idx].mean(0)- results['ctrl'])[0]
                if np.isnan(val):
                    val = 0
            else:
        ...
        if pert != 'ctrl':
            ctrl_de = results['ctrl_de'][p_idx][0,:].reshape(-1)
            
            for m, fct in metric2fct.items():
                if m == 'pearson':
                    val = fct(results['pred_de'][p_idx].mean(0) - ctrl_de, results['truth_de'][p_idx].mean(0) - ctrl_de)[0]
                    if np.isnan(val):
                        val = 0
                else:
