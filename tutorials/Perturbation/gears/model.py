...
pretrained_model = 'CellFM'
dataset = 'norman'

print(f'loading embedding from {pretrained_model}')
ctrl_emb_size = 512
ctrl_emb = torch.tensor(np.load(f'../../../dataset/{pretrained_model}-{dataset}.npy'))
ctrl_emb_size = ctrl_emb.shape[-1]
print(ctrl_emb.shape)
...
class GEARS_Model(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        # self.lin = nn.Linear(ctrl_emb_size, hidden_size)
        self.pert_base_trans = nn.ReLU()
        self.transform = nn.ReLU()
        self.emb_trans_v2 = MLP([1536, 1024, 512], last_layer_act='ReLU')
        ...
        if args['model_type'] == 'maeautobin':
            ...
            from modules.encoders import MAEAutobinencoder
            self.singlecell_model = MAEAutobinencoder(args, hidden_size=512)
            self.pretrained = True
        else:
    ...
    def forward(self, data):
        ...
        if self.no_perturb:
            ...
            if self.pretrained:
                if self.args['model_type'] in ['maeautobin', 'API']:
                    ...
                    emb = emb[:,:self.num_genes,:].contiguous()
                ...
                elif self.args['model_type'] in ['emb']:
                    x = x.reshape(num_graphs, self.num_genes+1)[:,:-1]
                    emb = ctrl_emb[data.z.detach().cpu()].to(self.args['device'])
                    emb = emb.view(-1, ctrl_emb_size)
                    pos_emb = self.emb_pos(torch.LongTensor(list(range(self.num_genes))).repeat(num_graphs, ).to(self.args['device']))
            else:
                x = x.reshape(num_graphs, self.num_genes+1)[:,:-1]
            ...
            if pretrained_model == 'None':
                ...
                base_emb = self.emb_trans_v2(emb)
