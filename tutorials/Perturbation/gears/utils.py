...
def create_cell_graph_for_prediction(X, z, pert_idx, pert_gene):
    ...
    return Data(x=torch.Tensor(X).T, z=z, pert_idx = pert_idx, pert=pert_gene)
...
def create_cell_graph_dataset_for_prediction(pert_gene, ctrl_adata, gene_names, device, num_samples = 300):
    ...
    if 'total_count' in ctrl_adata.obs:
    ...
    # Create cell graphs
    cell_graphs = [create_cell_graph_for_prediction(X, ridx[i], pert_idx, pert_gene).to(device) for i,X in enumerate(Xs)]
    return cell_graphs
