...
from cellot_model.models import load_autoencoder_model
from cellot_model.utils import load_config
from cellot_model.data.utils import cast_dataset_to_loader
from cellot_model.utils.helpers import nest_dict
import mindspore as ms
from cellot_model.models.cellfm import load_cellfm_model
import os
import scanpy as sc
...
def load_cell_data(
    ...
    **kwargs
    ...
    if "ae_emb" in config.data:
        ...
        print('loading cellfm embedding')
        data_path = f'./dataset/{config.data.target}_cellfm_50.h5ad'
        if os.path.exists(data_path):
            ...
            gene=np.array([geneset[i] for i in data.var['gene_short_name']]).astype(np.int32)
            gene = gene.reshape(1,-1)
            zero_idx = np.ones_like(gene).astype(np.float32)
            cls_emb = []

            x = np.array(data.X.todense()).astype(np.float32)
            zero_idx = ms.Tensor(zero_idx)
            gene = ms.Tensor(gene)
            inputs = ms.Tensor(x)
            for i in tqdm(range(len(data))):
                value = inputs[i:i+1, :]
                _,_,cls_token=cellfm_model.forward(
                    value,gene,zero_idx
                )
                cls_emb.append(cls_token.asnumpy())
            cls_emb = np.concatenate(cls_emb, axis=0)
            data = anndata.AnnData(
                cls_emb,
                obs=data.obs.copy(),
                uns=data.uns.copy(),
