...
from cellot_model.losses.mmd import mmd_distance
from cellot_model.models.ae import compute_scgen_shift
from cellot_model.transport import transport
from cellot_model.utils import load_config
from cellot_model.utils.loaders import load, load_data, load_model
...
def load_conditions(expdir, where, setting, embedding=None):
    ...
    if config.model.name == "identity":
        ...
        if config.model.name in ['cellot']:
            model = (model[0].to("cpu"), model[1].to("cpu"))
        else:
            model = model.to("cpu")
